...
def load_data(img_folder, labels_path, classes_type, model_type, images_name):
...
def visualize_tsne(datasets, n_limit, output_file, categories, apply_correction=False, use_background=False):
...
def parse_args():
    ...
    parser.add_argument('--output_folder', type=str, default='./proposals_features_t-sne_.pdf', help='Folder where to save the output file.')
...
if __name__ == "__main__":
    ...
    if os.path.exists(args.extracted_features):
        ...
        all_data = load_data(args.extracted_features, args.labels, args.classes, args.model, images_name)

        print("TSNE")
        visualize_tsne(all_data, args.n_limit, args.output_folder, categories)
    else:
